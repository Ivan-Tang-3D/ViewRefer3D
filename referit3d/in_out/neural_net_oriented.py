...
def compute_auxiliary_data(referit_data, all_scans, args):
    ...
    if args.vocab_file:
        ...
        train_tokens2 = referit_data[referit_data.is_train]['tokens2']
        for sentence in train_tokens2:
            sentence_token = split_sentences(sentence)
            for k in range(len(sentence_token)):
                extra_tokens.append(sentence_token[k])
        train_tokens3 = referit_data[referit_data.is_train]['tokens3']
        for sentence in train_tokens3:
            sentence_token = split_sentences(sentence)
            for k in range(len(sentence_token)):
                extra_tokens.append(sentence_token[k])
        train_tokens4 = referit_data[referit_data.is_train]['tokens4']
        for sentence in train_tokens4:
            sentence_token = split_sentences(sentence)
            for k in range(len(sentence_token)):
                extra_tokens.append(sentence_token[k])
        tokens = []
        ...
        vocab = build_vocab(tokens+extra_tokens, args.min_word_freq)
