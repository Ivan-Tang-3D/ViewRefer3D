...
def remove_punctuation_and_join(sentence):
    # 使用正则表达式替换掉所有非单词字符 (非字母数字或下划线) 为一个空格
...
class ListeningDataset(Dataset):
    ...
    def get_reference_data(self, index):
        ...
        #NOTE: for gpt agg
        multiview_text = ref['multiview_text']
       
        tokens2 = remove_punctuation_and_join(ref['tokens2'])
        tokens3 = remove_punctuation_and_join(ref['tokens3'])
        tokens4 = remove_punctuation_and_join(ref['tokens4'])
        
        return scan, target, tokens, is_nr3d, scan_id, multiview_text, tokens2, tokens3, tokens4
    ...
    def __getitem__(self, index):
        res = dict()
        scan, target, tokens, is_nr3d, scan_id, multiview_text, tokens2, tokens3, tokens4 = self.get_reference_data(index)
        ...
        if torch.rand(1).item() > 0.5:
            ...
            box_info[:, 0] = -1 * box_info[:, 0]

        if torch.rand(1).item() > 0.5:
            # Flipping along the XZ plane
            samples[:, :, 1] = -1 * samples[:, :, 1]
            box_info[:, 1] = -1 * box_info[:, 1]
        ...
        #NOTE: gzy for gpt
        res['multiview_text'] = multiview_text
        res['tokens2'] = tokens2
        res['tokens3'] = tokens3
        res['tokens4'] = tokens4
        ##############
